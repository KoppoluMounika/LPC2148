...
// ADC initialization (P0.28 = AD0.1)
void adc_init(void) {
...
int main(void) {
    ...
    while(1) {
        ...
        temp = voltage * 100.0;             // Convert voltage to °C (10mV/°C)

        lcd_cmd(0x80); // Move to first line
        sprintf(buffer,"Temp: %.2f C   ", temp); // Extra spaces to clear old digits
        lcd_string(buffer);
